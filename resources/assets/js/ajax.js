$(document).ready(function () {
    $(document).on('click', '.hidden_item', function () {
        ...
        // if (alert_pro('you really want to hidden..?') == true) {
        ...
        $.ajax({
            ...
            $this.addClass('btn-success show_item').html('<i class="fa fa-eye"></i>').attr({
                title: 'show item',
                href_page: window.location.origin + '/show_status_' + filename + '/' + id,
            });
...
$(document).ready(function () {
    $(document).on('click', '.show_item', function () {
        ...
        var url = window.location.pathname;
        var filename = url.substring(url.lastIndexOf('/') + 1);
        // if (alert_pro('you really want to show item..?') == true) {
        ...
        $.ajax({
            ...
            $this.addClass('btn-warning hidden_item').html(' <i class="fa fa-eye-slash"></i>').attr({
                title: 'hidden item',
                href_page: window.location.origin + '/hidden_status_' + filename + '/' + id,
            });
        })
        // }
    });
